...
Lab-only traffic. No external connections and no real credentials.
Standard library only.

Run:
  python tools/generate_wireshark_ctf.py

Creates pcap/ALFA.pcap, BRAVO.pcap, CHARLY.pcap and DELTA.pcap.
Starting Wireshark filter:
...
TEAMS = {"ALFA": (11, "02 00 00 00 00 11"), "BRAVO": (12, "02 00 00 00 00 12"), "CHARLY": (13, "02 00 00 00 00 13"), "DELTA": (14, "02 00 00 00 00 14")}
...
NOISE = ["heartbeat node=LAB-01 status=ok", "telemetry cpu=41 memory=62", "dns query=updates.dicib.test answer=10.10.10.81", "healthcheck service=training status=200"]


def ip_bytes(ip): return bytes(int(x) for x in ip.split('.'))
...
def tcp_packet(src_ip, dst_ip, sport, dport, seq, ack, flags, payload, src_mac, dst_mac):
    total = 40 + len(payload); ident = (seq ^ ack) & 0xffff
    iph = struct.pack('!BBHHHBBH4s4s', 0x45, 0, total, ident, 0, 64, 6, 0, ip_bytes(src_ip), ip_bytes(dst_ip))
    iph = struct.pack('!BBHHHBBH4s4s', 0x45, 0, total, ident, 0, 64, 6, checksum(iph), ip_bytes(src_ip), ip_bytes(dst_ip))
    off = (5 << 12) | flags
    ...
    th = struct.pack('!HHIIHHHH', sport, dport, seq, ack, off, 65535, checksum(pseudo + th + payload), 0)
    return dst_mac + src_mac + struct.pack('!H', 0x0800) + iph + th + payload


def evidence(team, n):
    a = answer(team, n).encode('utf-8')
    if n == 1: return 'BASE64', base64.b64encode(a).decode(), None
    if n == 2: return 'HEX', a.hex(), None
    if n == 3: return 'URL', ''.join('%%%02X' % b if b in b'{} ' else chr(b) for b in a), None
    if n == 5:
        import codecs
...
def pair(team, n, client_ip, client_mac, base):
    sport = 40000 + n; seq = 100000 + n * 1000; ack = 500000 + n * 1000
    uri = f'/event/{team.lower()}/{n:02d}?clue=event-{n:02d}'
    req = (f'GET {uri} HTTP/1.1\r\nHost: ctf.dicib.test\r\nUser-Agent: DICIB-Wireshark-Lab/1.0\r\nAccept: */*\r\nConnection: close\r\n\r\n').encode()
    mode, ev, ev2 = evidence(team, n)
    body = f'DICIB LAB EVENT {n:02d}\nTEAM={team}\nOBJECTIVE={team}-{n:02d}\nCLUE={CLUES[n-1]}\nENCODING={mode}\nEVIDENCE={ev}'
    if ev2 is not None: body += f'\nEVIDENCE-2={ev2}'
    body += '\n'
    body = body.encode('utf-8')
    headers = f'HTTP/1.1 200 OK\r\nContent-Type: text/plain; charset=utf-8\r\nContent-Length: {len(body)}\r\nX-CTF-Clue: event-{n:02d}\r\n'
    if n == 12: headers += f'X-CTF-Part-A: {ev}\r\nX-CTF-Part-B: {ev2}\r\n'
    if n == 14: headers += 'X-CTF-Review: REVISAR-CORREO\r\n'
    if n == 16: headers += 'X-CTF-Review: BANCO-UENO\r\n'
    if n == 17: headers += 'X-CTF-Language: GUARANI\r\n'
...
def generate(team):
    last, mac_hex = TEAMS[team]; client_ip = f'10.10.10.{last}'; client_mac = bytes.fromhex(mac_hex); frames=[]
    ...
    for n in range(1, 26):
        ...
        frames.extend(pair(team, n, client_ip, client_mac, base))
    OUT.mkdir(parents=True, exist_ok=True); p=OUT/f'{team}.pcap'
    with p.open('wb') as f:
        f.write(struct.pack('<IHHIIII', 0xA1B2C3D4, 2, 4, 0, 0, 65535, 1))
        for i, frame in enumerate(frames): f.write(struct.pack('<IIII', base+i, (i*137)%1000000, len(frame), len(frame)) + frame)
    print(f'created {p} ({len(frames)} packets)')

if __name__ == '__main__':
    for team in TEAMS: generate(team)
