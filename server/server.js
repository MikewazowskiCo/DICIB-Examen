...
async function dashboardState(){
 const out=[];for(const [id,rank,name] of PLAYERS)out.push({id,rank,name,...await playerState(id)});
 out.sort((a,b)=>b.score-a.score||b.solved-a.solved||a.id.localeCompare(b.id));return out;
}
...
app.post('/api/login',(req,res)=>{const user=String(req.body?.user||'').trim().toUpperCase();const password=String(req.body?.password||'');if(user!==ADMIN_USER||!ADMIN_PASSWORD||password!==ADMIN_PASSWORD)return res.status(401).json({error:'Credenciales de administrador incorrectas'});res.json({token:newToken(user,'admin'),user,role:'admin'});});
app.post('/api/player/start',(req,res)=>{const id=String(req.body?.playerId||'').trim().toUpperCase();const p=playerMap.get(id);if(!p)return res.status(400).json({error:'Operador no válido'});res.json({token:newToken(id,'player'),user:id,role:'player',player:p});});
...
app.post('/api/questions/answer',auth,playerOnly,async(req,res)=>{const p=playerMap.get(req.session.user);const q=QUESTIONS.find(x=>x.id===String(req.body?.questionId||'').trim().toUpperCase());const choice=Number(req.body?.choice);if(!q||!Number.isInteger(choice)||choice<0||choice>=q.options.length)return res.status(400).json({error:'Pregunta o respuesta no válida'});const already=await pool.query(`SELECT flag FROM blackout_events WHERE player_id=$1 AND question_id=$2 AND type='CORRECT' LIMIT 1`,[p.id,q.id]);if(already.rowCount)return res.json({correct:true,already:true,flag:already.rows[0].flag,state:await playerState(p.id)});if(choice===q.answer){const flag=makeFlag(p,q);await pool.query(`INSERT INTO blackout_events(player_id,type,question_id,flag,answer_index) VALUES($1,'CORRECT',$2,$3,$4) ON CONFLICT DO NOTHING`,[p.id,q.id,flag,choice]);return res.json({correct:true,flag,points:100,state:await playerState(p.id)});}await pool.query(`INSERT INTO blackout_events(player_id,type,question_id,answer_index) VALUES($1,'WRONG',$2,$3)`,[p.id,q.id,choice]);res.json({correct:false,penalty:50,state:await playerState(p.id)});});

// Ranking público: no requiere iniciar sesión. Solo expone datos de competencia.
app.get('/api/ranking',async(_req,res)=>{try{const players=await dashboardState();res.json({updatedAt:new Date().toISOString(),players:players.map((p,i)=>({position:i+1,id:p.id,name:p.name,score:p.score,solved:p.solved,total:p.total,percent:p.percent}))});}catch(error){console.error('ranking error',error);res.status(503).json({error:'Ranking temporalmente no disponible'});}});

app.get('/api/admin/dashboard',auth,adminOnly,async(_req,res)=>{const players=await dashboardState();const totals=players.reduce((a,p)=>(a.score+=p.score,a.solved+=p.solved,a.errors+=p.errors,a),{score:0,solved:0,errors:0});const {rows:events}=await pool.query(`SELECT player_id,type,question_id,flag,created_at FROM blackout_events ORDER BY id DESC LIMIT 40`);res.json({players,totals,events});});
